import { auth } from "@clerk/nextjs/server"
import { Liveblocks } from "@liveblocks/node"

export async function POST(request: Request) {
  try {
    const secretKey = process.env.LIVEBLOCKS_SECRET_KEY

    if (!secretKey) {
      console.error("[Liveblocks Auth Error] LIVEBLOCKS_SECRET_KEY is missing from environment variables.")
      return Response.json(
        { error: "LIVEBLOCKS_SECRET_KEY is missing in environment variables" },
        { status: 500 }
      )
    }

    const liveblocks = new Liveblocks({ secret: secretKey })

    // Get the current user from Clerk session claims
    const { userId, sessionClaims } = await auth()

    if (!userId) {
      return Response.json({ error: "Unauthorized" }, { status: 401 })
    }

    // Parse the room ID from the request body sent by the Liveblocks client
    let roomId: string | undefined
    try {
      const body = await request.json()
      roomId = body?.room as string | undefined
    } catch {
      // Body may be empty or unparseable, continue gracefully
    }

    // Build user info from session claims
    const claims = sessionClaims as Record<string, unknown> | undefined
    const firstName = (claims?.firstName as string) ?? ""
    const lastName = (claims?.lastName as string) ?? ""
    const username = (claims?.username as string) ?? ""
    const email = (claims?.email as string) ?? ""
    const imageUrl = (claims?.imageUrl as string) ?? ""

    const name = firstName
      ? `${firstName} ${lastName}`.trim()
      : username || email || "Anonymous User"

    // Start a Liveblocks session
    const session = liveblocks.prepareSession(userId, {
      userInfo: {
        name,
        avatar: imageUrl,
      },
    })

    // Grant the user read + write access to the requested room
    if (roomId) {
      session.allow(roomId, session.FULL_ACCESS)
    }

    // Authorize the session
    const { status, body: authBody } = await session.authorize()

    return new Response(authBody, {
      status,
      headers: {
        "Content-Type": "application/json",
      },
    })
  } catch (error) {
    const errorMsg = error instanceof Error ? error.message : String(error)
    console.error("[Liveblocks Auth Error]", error)
    return Response.json({ error: errorMsg }, { status: 500 })
  }
}
